...
st_css()
st_options(lang = "en",
           footnote = "Note: Descriptives presented only for wave 1",
           bootstrap.css = F,
...
df_study1 <- df_study_long_t7_comunas %>% 
  dplyr::select(ola, idencuesta,
...
df_study1$ess <- 
sjlabelled::set_label(x = df_study1$ess, 
                      label = "Subjective social status")

df_study1$ing_pc <- 
sjlabelled::set_label(x = df_study1$ing_pc, 
...
df<- dfSummary(x = dplyr::select(df_study1, covid_economia, covid_beneficios, 
                                 merit_talent, merit_effort, des_perc, 
                                 educ, ing_pc, ess, edad, sexo, pos_id),
               plain.ascii = FALSE,
               style = "grid",
               tmp.img.dir = "/tmp",
               graph.magnif = TRUE,
...
ggsave(alluvial_justeduc, file="output/graphs/alluvial_dep.png", height = 7, width = 9)
```
...
df_study1 <- df_study_long_t7_comunas %>% dplyr::select(idencuesta, ola, ponderador_long_total, segmento, estrato,
                                               just_educ, covid_economia, covid_beneficios, merit_talent, merit_effort, des_perc, 
                                               educ, ing_pc, ess, edad, sexo, pos_id) %>% na.omit()
# Full sample______________________________________________________________________
mat_full<- 
df_study1 %>%
   filter(ola==7) %>%
...
rownames(cor_full) <- c("A. Justification of educational inequality",
                        "B. Economy more important than health",
                        "C. Receive state benefits",
                        "D. Inequality perception",
                     "E. People are rewarded for their efforts",
                     "F. People are rewarded for their intelligence")
#set Column names of the matrix
colnames(cor_full) <-c("(A)", "(B)","(C)","(D)", "(E)", "(F)")
#Plot the matrix using corrplot
png("output/graphs/corrplot.png",width=700,height=600, pointsize = 12)
corrplot::corrplot(cor_full,
  ...
  na.label = "-")
# Agregar caption
grid::grid.text("Source: own elaboration with data from ELSOC 2023 (n = 1322)", x = 0.99, y = 0.1, just = "right", gp = grid::gpar(fontsize = 12, col = "black"))
dev.off()
# mat_1<- 
