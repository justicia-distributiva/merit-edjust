...
sjlabelled::set_label(x = df_study1$covid_economia, 
                      label = "It is more important to give priority to economic activity than to the health of the population")
...
df<- dfSummary(x = dplyr::select(df_study1, ola, covid_economia, covid_beneficios, merit_talent, merit_effort, des_perc, educ, quintil1, ess, edad, sexo, pos_id),
               plain.ascii = FALSE,
               style = "grid",
               tmp.img.dir = "/tmp",
               graph.magnif = TRUE,
...
df_study1 <- df_study_long_t7_comunas %>% dplyr::select(idencuesta, ola, ponderador_long_total, segmento, estrato,
                                               just_educ, des_perc, merit_effort, merit_talent,covid_economia, covid_beneficios,
...
alluvial_justeduc <- ggplot(datos.justeduc, aes(x = ola, fill = just_educ, stratum = just_educ,
  ...
  labs(caption = "Source: own elaboration with pooled data from ELSOC 2016-2023 (n = 7894)")
...
from ELSOC 2016-2023 (n = 7894)", x = 0.01, y = 0.05, just = "left", gp = grid::gpar(fontsize = 10, col = "black"))
dev.off()
