...
df_study1 <- 
  ...
  select(in_ansied,in_simpat,in_valfam,in_valami,in_amigcl,ola,idencuesta,
...
sjmisc::frq(df_study1$in_amreal)
sjmisc::frq(as.numeric(df_study1$in_amreal))
sjmisc::frq(df_study1$educ)

levels(df_study1$ola) <- c("2016","2017","2018","2019","2021")  
levels(df_study1$educ) <- c("Universitaria","Técnica","Media","Básica")  

#Paso 1
#dropeamos el nivel -999 de la variable:
df_study1$in_amreal1 <- car::recode(df_study1$in_amreal,
                                    "c('Strongly disagree','Disagree')='Desacuerdo';
                                    'Neither disagree nor agree'='Ni de acuerdo, ni desacuerdo';
                                    c('Agree','Strongly agree')='Deacuerdo'")  

df_study1$in_amreal1 <- factor(df_study1$in_amreal1,
                               labels =  c('Deacuerdo',
                                          'Ni de acuerdo, ni desacuerdo',
                                          'Desacuerdo'))  


a1 <- data.frame(table(haven::as_factor(df_study1$in_amreal1),
                       haven::as_factor(df_study1$ola),
                       haven::as_factor(df_study1$idencuesta)))

a1 <- a1 %>% filter(Var2 %in% c(2016,2018,2021))

 
a1 <- a1 %>% dplyr::filter(Freq>0)
names(a1) <- c("g_civico","ola","folio","Freq")
a2 <- a1 %>% group_by(ola) %>% transmute(porcentaje=Freq/sum(Freq)) 
a2$ola <- NULL
datos.d.1<- cbind(a1,a2)
datos.d.1 <- na.omit(datos.d.1)
#Paso 2
b1 <- data.frame(table(haven::as_factor(df_study1$in_amreal1),
                       haven::as_factor(df_study1$ola)))

b1 <-b1 %>% filter(Var2 %in% c(2016,2018,2021))


names(b1) <- c("g_civico","ola","Freq")
b1 <- b1 %>% dplyr::filter(Freq>0)
b2 <- b1 %>% 
  group_by(ola) %>% 
...
dim<- sjmisc::frq(datos.d.1$folio) %>% data.frame() %>% filter(frq==2) %>% dim()
#-----------------------------------------------------------------------------#
threat <- ggplot(datos.d.1, aes(x = ola, fill = g_civico, stratum = g_civico,
                             alluvium = folio, y = porcentaje)) +
  ggalluvial::geom_flow(alpha = .66) + 
  ggalluvial::geom_stratum(linetype = 0) +
  scale_y_continuous(labels = scales::percent) + 
  # scale_x_discrete(labels=c(2019,,2))+  
  ylab(label = NULL) +
  xlab(label = NULL) + 
  ...
  geom_text(data = etiquetas.d.1, 
            ...
            size = 4,
            color = rep('white'))+
  ggtitle("Grado de acuerdo: Con llegada de migrantes aumenta el desempleo") 
  labs(caption =  paste0("Fuente: Elaboración propia en base a ELSOC (n = ",
                         dim[1],")"))
threat  
...
#Paso 1
#dropeamos el nivel -999 de la variable:
df_study1$in_amigcl1 <- car::recode(df_study1$in_amigcl,
                                    "c('Strongly disagree','Disagree')='Desacuerdo';
                                    'Neither disagree nor agree'='Ni de acuerdo, ni desacuerdo';
                                    c('Agree','Strongly agree')='Deacuerdo'")  

df_study1$in_amigcl1 <- factor(df_study1$in_amigcl1,
                               labels =  c('Deacuerdo',
                                          'Ni de acuerdo, ni desacuerdo',
                                          'Desacuerdo'))  

  
a1 <- data.frame(table(haven::as_factor(df_study1$in_amigcl1),
                       haven::as_factor(df_study1$ola),
                       haven::as_factor(df_study1$idencuesta)))
...
datos.d.1 <- na.omit(datos.d.1)
#Paso 2
b1 <- data.frame(table(haven::as_factor(df_study1$in_amigcl1),
                       haven::as_factor(df_study1$ola)))
...
dim<- sjmisc::frq(datos.d.1$folio) %>% data.frame() %>% filter(frq==2) %>% dim()
#-----------------------------------------------------------------------------#
amigos <- ggplot(datos.d.1, aes(x = ola, fill = g_civico, stratum = g_civico,
                             alluvium = folio, y = porcentaje)) +
  ggalluvial::geom_flow(alpha = .66) + 
  ggalluvial::geom_stratum(linetype = 0) +
  scale_y_continuous(labels = scales::percent) + 
  ...
  geom_text(data = etiquetas.d.1, 
            ...
            size = 4,
            color = rep('white'))+
  ggtitle("Grado de acuerdo: Que migrantes tengan amigos chilenos") 
  labs(caption =  paste0("Fuente: Elaboración propia en base a ELSOC (n = ",
                         dim[1],")"))
amigos 
...
df_study1$in_amreal1 <- car::recode(df_study1$in_amreal,
                                    "c('Strongly disagree','Disagree','Neither disagree nor agree')='Desacuerdo';
                                    c('Agree','Strongly agree')='De acuerdo'")  

df_study1$in_amreal1 <- factor(df_study1$in_amreal1,
                               labels =  c('De acuerdo: desempleo aumenta',
                                          'Desacuerdo'))  
 

df_study1$in_simpat1 <- 
  car::recode(as_numeric(df_study1$in_simpat),
              "1:2='Ninguna o poca simpatía';3:5='Simpatía'")  


df_study1$in_simili1 <- 
  car::recode(as_numeric(df_study1$in_simili),
              "1:3='Ninguna o poca similitud';4:5='Similaridad'")  

a1 <- 
prop.table(table(df_study1$in_amreal1,df_study1$ola),margin = 2) %>% 
  as.data.frame() %>% 
  filter(Var1 == "De acuerdo: desempleo aumenta")  
a2 <- 
prop.table(table(df_study1$in_simpat1,df_study1$ola),margin = 2) %>% 
  as.data.frame() %>% 
  filter(Var1 == "Ninguna o poca simpatía")

a3 <- 
prop.table(table(df_study1$in_simili1,df_study1$ola),margin = 2) %>% 
  as.data.frame() %>% 
  filter(Var1 == "Ninguna o poca similitud")


df<- rbind(a1,a2,a3)
df$Var1 <- 
  factor(df$Var1,
         levels =c("Ninguna o poca simpatía","Ninguna o poca similitud",
                   "De acuerdo: desempleo aumenta") )


ggplot(data=df, aes(x=Var2, y=Freq, color = Var1,
                    group=Var1,
                    label = paste0(round((Freq)*100,2),'%'))) +
  geom_line()+
  geom_point() +
  ...
  theme(legend.position="bottom",
        legend.title=element_blank())
ggsave(filename = 'figure_emol.png',device = 'png',
       plot = last_plot(),path = "output/images",
       units = "cm",width = 10,height = 10,scale = 2)

ggsave(filename = 'figure_emol.pdf',device = 'pdf',
       plot = last_plot(),path = "output/images",
       units = "cm",width = 10,height = 10,scale = 2)
```
...
e.3 <- ggplot(subset.e.3,aes(y = porcentaje, x = ola, color = variable, group = variable,
    ...
    ylab(label = NULL) +
    xlab(label = NULL) +
    scale_color_viridis_d(begin = .2, end = .7, direction = 1, option = 'viridis') +
    facet_wrap(.~zona)+
    geom_text(vjust = -0.8,
...
pdf(here::here("output/images/figure1.pdf"),
    # units = "cm",
    # res = 100,
    width = 15,
    height = 15)
...
pdf(file=here::here("output/images/figure2.pdf"),width=13, height=11)
corrplot::corrplot(cor_full,
  title = "\nFigure 2: Polychoric correlation matrix for Conviviality, Identity and Threat items",
  method = "color",
  addCoef.col = "#000390",
