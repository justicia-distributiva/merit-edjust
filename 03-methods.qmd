...
For testing the hypothesis regarding status we included educational level and household income. Also, we included subjective social status as it has been argued that perceived social status is a relevant predictor of attitudes toward economic inequality [@schneider_poverty_2015;@castillo_meritocracia_2019]. In addition, subjective measures can complement objective measures in predicting life chances [@oesch_subjective_2023], which are arguably connected to how individuals experience economic inequality. In this regard, it has been argued that how people form their views on their standing in society results from experiences with direct socioeconomic circumstances and social comparison processes with reference groups [@Condon2020]. Finally, political identification on a left-right scale, age groups, and gender were included as control variables. @tbl-descriptives shows the whole set of independent variables, the corresponding items,  response categories, and frequencies.
